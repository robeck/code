...
%macro FM_Rolling(indata= ,outdata= ,depvar= ,indvars= , stdm= , eddm= ,cusip= ,bydate= ,reqnum=20,reqdate=60);
	...
	data &indata;set &indata;if bond_excess_return='.' then delete;run;
    /*首先对输入的数据做处理*/
	data &indata(keep=&cusip &indvars &depvar &bydate date);
	set &indata;
	by &cusip;
	...
	proc datasets lib=c nolist; delete &outdata; quit;
	%do dm = &stdm %to &eddm %by 30;
		data work._tm_1; set &indata (keep=&cusip &bydate &depvar &indvars); 
...
%macro FM_cross(rawdata=,tmdata=,fmbeta=,depvar=,indvars=,kvar=none,lag=3,cusip=complete_cusip,bytm=dm,bydate=delta_month,bc='yes',bc2='yes');
